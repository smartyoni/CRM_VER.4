...
| 데이터 | 가능한 헤더 |
|--------|-----------|
| 접수일 | 접수일, created_at, createdAt, 등록일 |
...
### 1. CSV 파일 준비
```
접수일,건물명,호실명,매물유형,구분,금액,입주일,소유자,소유자번호,임대차정보,점주번호,메모
2024-11-01,강남 오피스텔,501호,매매,오피스텔,50000,2024-12-01,김철수,010-1234-5678,전월세,010-9876-5432,한강뷰 좋음
2024-10-25,서초 상가,2층,임대,상가,30000,2024-11-15,이영희,02-123-4567,3년 임차,010-5555-6666,최신식 인테리어
```
...
### 데이터 형식
- **접수일**: YYYY-MM-DD 형식 (예: 2024-11-01) - CSV에 없으면 현재 시간으로 자동 설정
- **금액**: 숫자만 입력 (예: 50000, 30000) - "만원" 단위 자동 적용
- **날짜 (입주일)**: YYYY-MM-DD 형식 (예: 2024-12-01)
...
### 유효성 검사
- 건물명 필수 (필수)
- 접수일: CSV에 있으면 그 값, 없으면 현재 시간으로 자동 설정
- 금액은 자동으로 숫자로 변환
- 빈 필드는 기본값으로 설정:
  - 매물유형 기본값: '매매'
  - 구분 기본값: '오피스텔'
  - 금액 기본값: 0
  - 접수일 기본값: 현재 시간 (ISO 8601 형식)
  - 그 외 필드: 빈 문자열
